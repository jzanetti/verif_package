'''main script for ifs download and decoding'''
import argparse
...
def setup_parser():
    ...
    parser.add_argument('--download_ifs', dest='download_ifs',
                        help='download ifs data from S3 ',
                        action='store_true')
    ...
    # ------------------------------------------
    parser.add_argument('--ifs_dir_on_s3', dest='ifs_dir_on_s3',
                        help='ifs_dir from S3')
    ...
    return parser.parse_args(['--work_dir', '/home/szhang/nwp_verification', 
                              ...
                              '--analysis_hour_interval', '12',
                              '--fcst_length', '6', 
                              '--role_name', 'amps-research-framework',
                              '--ifs_dir_on_s3', 's3://metservice-research-us-west-2-vault/incoming-external-data/ecmwf/det'])
...
def main_radar():
    ...
    if args.download_ifs:
        data_info = data_download_upload.download_model(args, model_type='ifs')
        data_download_upload.convert_unique_dict_to_pandas(
            args.work_dir, data_info, 'ifs')
    ...
    if args.run_verification:
        if 'rainfall' in args.verif_types:
            # project model data to the radar grids
            data_download_upload.write_model_rainfall_on_radar_grids(
                args.work_dir, 'ifs', accumulation_length=3)
